...
export default function App() {
  ...
  const [driveFileId, setDriveFileId] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('tracker_drive_file_id');
    }
    return null;
  });
  ...
  const syncFromDrive = async (token: string) => {
    try {
      ...
      // Use existing file ID if available to speed up sync
      ...
      if (!fileId) {
        ...
        if (file) {
          ...
          localStorage.setItem('tracker_drive_file_id', file.id);
        }
      }

      if (fileId) {
        const content = await googleDriveService.getFileContent(token, fileId);
        if (content) {
      ...
      // Even on error, we mark initial sync as "done" so the user can at least see the app
      setIsInitialSyncDone(true);
    } finally {
      setIsSyncing(false);
    }
  ...
  return (
    <div className="max-w-md mx-auto h-[100dvh] bg-slate-50 flex flex-col shadow-2xl relative overflow-hidden font-sans">
      {/* Cloud Connection Screen - Shown if logged in but Drive not connected */}
      <AnimatePresence>
        {isFirebaseMode && user && !googleAccessToken && !authLoading && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[260] bg-white flex flex-col items-center justify-center p-8 text-center"
          >
            <div className="w-20 h-20 bg-amber-500 rounded-3xl flex items-center justify-center mb-8 shadow-xl -rotate-3">
            ...
            <h1 className="text-3xl font-black text-slate-800 uppercase tracking-tighter mb-4 leading-none">
              Cloud Sync<br/><span className="text-amber-600">Required</span>
            </h1>
            <p className="text-slate-500 text-sm mb-12 max-w-[260px] leading-relaxed">
              To keep your data safe and synced across devices, please connect your Google Drive account.
            </p>
            
            <button 
              onClick={handleGoogleLogin}
              className="w-full py-4 bg-amber-600 text-white rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl hover:bg-amber-700 active:scale-95 transition-all"
            >
              <Settings size={20} />
            ...
            <button 
              ...
              className="mt-6 text-[10px] text-slate-400 uppercase tracking-widest font-black hover:text-rose-500 transition-colors"
            >
      ...
      {/* Syncing Overlay - Covers app until initial data is ready */}
      <AnimatePresence>
        {user && googleAccessToken && !isInitialSyncDone && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter mb-2">Recovering Records</h2>
            <p className="text-slate-500 text-sm text-center max-w-[200px] leading-relaxed">
              Please wait while we securely sync your data from Google Drive...
            </p>
      ...
      {/* Auth Screen - Only shown if not logged in */}
      <AnimatePresence>
        {isFirebaseMode && !user && !authLoading && (
          <motion.div 
            ...
            className="fixed inset-0 z-[250] bg-white flex flex-col items-center justify-center p-8 text-center"
          >
            <div className="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center mb-8 shadow-xl rotate-3">
              <Wallet className="text-white" size={40} />
            </div>
            <h1 className="text-4xl font-black text-slate-800 uppercase tracking-tighter mb-4 leading-none">
              Expense<br/><span className="text-indigo-600">Tracker</span>
            </h1>
            <p className="text-slate-500 text-sm mb-12 max-w-[240px] leading-relaxed">
              Securely track your finances with automatic Google Drive cloud backup.
            </p>
            
            <button 
              onClick={handleGoogleLogin}
              className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl hover:bg-slate-800 active:scale-95 transition-all"
            >
              <Settings size={20} />
              Login with Google
            </button>
            ...
            <p className="mt-8 text-[10px] text-slate-400 uppercase tracking-widest font-bold">
              Your data stays private in your own Drive
            </p>
          </motion.div>
        )}
      </AnimatePresence>
